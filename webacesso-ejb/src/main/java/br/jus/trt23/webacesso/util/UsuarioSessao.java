...
public class UsuarioSessao implements Serializable {
    ...
    private MenuModel carregaMenu2(){
    ...
    private MenuModel carregaMenu() {
        MenuModel _menuModel = new DefaultMenuModel();

        //define o menu conforme o perfil ativo
        DefaultSubMenu funcionalidadeMenu;
        DefaultMenuItem itemMenu;
        for (Funcionalidade func : getListaFuncionalidades()) {
            //funcionalidadeMenu = new DefaultSubMenu("a");
            funcionalidadeMenu = new DefaultSubMenu(func.getDescricao());
            for (FuncionalidadeUrl funcUrl : func.getListaUrl()) {
                if (null == funcUrl.getLegenda()) {
                    itemMenu = new DefaultMenuItem("Nao definido");
                } else {
                    //itemMenu = new DefaultMenuItem("b");
                    itemMenu = new DefaultMenuItem(funcUrl.getLegenda());                    
                }
                itemMenu.setUrl(funcUrl.getViewId());
                funcionalidadeMenu.addElement(itemMenu);
            }
            _menuModel.addElement(funcionalidadeMenu);
        }
        return _menuModel;
    }
}
